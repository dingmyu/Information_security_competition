/*__________________*/
#include <stdio.h>
#include <string.h>
#define ITEMLENGTH 12
unsigned char cipher0[ITEMLENGTH * 2];
unsigned char password[ITEMLENGTH];
unsigned char cipher[ITEMLENGTH];
typedef unsigned char u8;
typedef unsigned int u32;
/*__________________*/
/*the state registers of LFSR*/
u32 LFSR_S0;
u32 LFSR_S1;
u32 LFSR_S2;
u32 LFSR_S3;
u32 LFSR_S4;
u32 LFSR_S5;
u32 LFSR_S6;
u32 LFSR_S7;
u32 LFSR_S8;
u32 LFSR_S9;
u32 LFSR_S10;
u32 LFSR_S11;
u32 LFSR_S12;
u32 LFSR_S13;
u32 LFSR_S14;
u32 LFSR_S15;

/*the registers of F */
u32 F_R1;
u32 F_R2;
/*the outputs of BitReorganitions*/
u32 BRC_X0;
u32 BRC_X1;
u32 BRC_X2;
u32 BRC_X3;

/*the s-boxes*/
u8 S0[256]={

};
u8 S1[256]={

};
/* the contants D*/
u32 EK_d[16] = {
0x44D7,0x26BC,0x626B,0x135E,0x5789,0x35E2,0x7135,0x09AF,
0x4D78,0x2F13,0x6BC4,0x1AF1,0x5E26,0x3C4D,0x789A,0x47AC
};
/*___________________*/
/* c =a+b mod 2^31-1*/
u32 AddM(u32 a,u32 b)
{
   u32 c = a+b;
   if(c&0x80000000)
   {
      c=(c&0x7FFFFFFF)+1;
    }
    return c;
}
/*LFSR with initialization mode */
#define MulByPow2(x,k) ((((x)<<k)|((x)>>(31-k)))&0x7FFFFFFF)
void LFSRWithInitializationMode(u32 u)
{
    u32 f,v;
    f=LFSR_S0;
    v=MulByPow2(LFSR_S0,8);
    f=AddM(f,v);
    v=MulByPow2(LFSR_S4,20);
    f=AddM(f,v);
    v=MulByPow2(LFSR_S10,21);
    f=AddM(f,v);
    v=MulByPow2(LFSR_S13,17);
    f=AddM(f,v);
    v=MulByPow2(LFSR_S15,15);
    f=AddM(f,v);
/* update the state*/
    LFSR_S0 = LFSR_S1;
    LFSR_S1 = LFSR_S2;
    LFSR_S2 = LFSR_S3;
    LFSR_S3 = LFSR_S4;
    LFSR_S4 = LFSR_S5;
    LFSR_S5 = LFSR_S6;
    LFSR_S6 = LFSR_S7;
    LFSR_S7 = LFSR_S8;
    LFSR_S8 = LFSR_S9;
    LFSR_S9 = LFSR_S10;
    LFSR_S10 = LFSR_S11;
    LFSR_S11 = LFSR_S12;
    LFSR_S12 = LFSR_S13;
    LFSR_S13 = LFSR_S14;
    LFSR_S14 = LFSR_S15;
    LFSR_S15 = u^f;
/* adjust LFSR_S15 if LFSR_S15 is zero*/
    if (LFSR_S15 == 0)
   {
       LFSR_S15=0x7FFFFFFF;
   }
}
/* LFSR with work mode*/
void LFSRWithWorkMode()
{
  u32 f,v;
  f=LFSR_S0;
  v=MulByPow2(LFSR_S0,8);
  f=AddM(f,v);
  v=MulByPow2(LFSR_S4,20);
  f=AddM(f,v);
  v=MulByPow2(LFSR_S10,21);
  f=AddM(f,v);
  v=MulByPow2(LFSR_S13,17);
  f=AddM(f,v);
  v=MulByPow2(LFSR_S15,15);
  f=AddM(f,v);
/*update the state*/
  LFSR_S0 = LFSR_S1;
  LFSR_S1 = LFSR_S2;
  LFSR_S2 = LFSR_S3;
  LFSR_S3 = LFSR_S4;
  LFSR_S4 = LFSR_S5;
  LFSR_S5 = LFSR_S6;
  LFSR_S6 = LFSR_S7;
  LFSR_S7 = LFSR_S8;
  LFSR_S8 = LFSR_S9;
  LFSR_S9 = LFSR_S10;
  LFSR_S10 = LFSR_S11;
  LFSR_S11 = LFSR_S12;
  LFSR_S12 = LFSR_S13;
  LFSR_S13 = LFSR_S14;
  LFSR_S14 = LFSR_S15;
  LFSR_S15 = f;
  if(LFSR_S15==0)
  {
	  LFSR_S15=0x7FFFFFFF;
  }
}
/*BitReorganization*/
void BitReorganization()
{
    BRC_X0 = ((LFSR_S15 & 0x7FFF8000)<<1)| (LFSR_S14 & 0xFFFF);
    BRC_X1 = ((LFSR_S11 & 0xFFFF)<<16)| (LFSR_S9 >>15);
    BRC_X2 = ((LFSR_S7 & 0xFFFF)<<16)| (LFSR_S5 >>15);
    BRC_X3 = ((LFSR_S2 & 0xFFFF)<<16)| (LFSR_S0 >>15);
}
#define ROT(a,k) (((a)<<k) | ((a)>>(32-k)))
/*L1*/
u32 L1(u32 X)
{
return (X^ROT(X,2)^ROT(X,10)^ROT(X,18)^ROT(X,24));
}
/*L2*/
u32 L2(u32 X)
{
return (X^ROT(X,8)^ROT(X,14)^ROT(X,22)^ROT(X,30));
}
#define MAKEU32(a,b,c,d) (((u32)(a)<<24) | ((u32)(b)<<16)|((u32)(c)<<8)|((u32)(d)))
#define MAKEU31(a,b,c) (((u32)(a)<<23)|((u32)(b)<<8)|(u32)(c))
/*F*/
u32 F()
{
    u32 W,W1,W2,u,v;
    W=(BRC_X0^F_R1)+F_R2;
    W1=F_R1+BRC_X1;
    W2=F_R2^BRC_X2;
    u=L1((W1<<16)|(W2>>16));
    v=L2((W2<<16)|(W1>>18));
    F_R1=MAKEU32(S0[u>>24],S1[(u>>16)&0xFF],S0[(u>>8)&0xFF],S1[u&0xFF]);
    F_R2=MAKEU32(S0[v>>24],S1[(v>>16)&0xFF],S0[(v>>8)&0xFF],S1[v&0xFF]);
    return W;
}
/*initialize*/
void Initialization(u8* k,u8* iv)
{
    u32 u,nCount;
/*expand key*/
    LFSR_S0 = MAKEU31(k[0],EK_d[0],iv[0]);
    LFSR_S1 = MAKEU31(k[1],EK_d[1],iv[1]);
    LFSR_S2 = MAKEU31(k[2],EK_d[2],iv[2]);
    LFSR_S3 = MAKEU31(k[3],EK_d[3],iv[3]);
    LFSR_S4 = MAKEU31(k[4],EK_d[4],iv[4]);
    LFSR_S5 = MAKEU31(k[5],EK_d[5],iv[5]);
    LFSR_S6 = MAKEU31(k[6],EK_d[6],iv[6]);
    LFSR_S7 = MAKEU31(k[7],EK_d[7],iv[7]);
    LFSR_S8 = MAKEU31(k[8],EK_d[8],iv[8]);
    LFSR_S9 = MAKEU31(k[9],EK_d[9],iv[9]);
    LFSR_S10 = MAKEU31(k[10],EK_d[10],iv[10]);
    LFSR_S11 = MAKEU31(k[11],EK_d[11],iv[11]);
    LFSR_S12 = MAKEU31(k[12],EK_d[12],iv[12]);
    LFSR_S13 = MAKEU31(k[13],EK_d[13],iv[13]);
    LFSR_S14 = MAKEU31(k[14],EK_d[14],iv[14]);
    LFSR_S15 = MAKEU31(k[15],EK_d[15],iv[15]);
/*set F_R1 and F_R2 to zero*/
    F_R1=0;
    F_R2=0;
    nCount=32;
    while(nCount >0)
    {
      BitReorganization();
      u=F()^BRC_X3;
      LFSRWithInitializationMode(u>>1);
      nCount --;
    }
}
void GenerateKeystream(u32* pKeystream,int KeystreamLen)
{
    int i;
    BitReorganization();
    F();/*discard the output of F*/
    LFSRWithWorkMode();
    for(i=0;i<KeystreamLen;i++)
    {
        BitReorganization();
        pKeystream[i]=F()^BRC_X3;
        //printf("%c",pKeystream[i]);
        LFSRWithWorkMode();
    }  
}
/*void getPw()
{
    FILE *fp;
    char stream[50];
    int i;
    fp=fopen("password.txt","r");
    fgets(stream,50,fp);
    for(i=0;i<ITEMLENGTH;i++)
    {
       password[i]=stream[i];
    }
    password[i]='\0';
    fclose(fp);
}*/
void getPw() {
    //int len = 0;
    //scanf("%d", &len);
    for(int i = 0; i < ITEMLENGTH; i++)
       scanf("%c", &password[i]);
}
void encrypt(unsigned char *key)
{
    int i=0;
    for (i=0;i<ITEMLENGTH;i++)
    {
       cipher[i]=key[i]^password[i]; 
    }  
}
int main()
{
    u32 Keystream[ITEMLENGTH/4];
    int i;
    unsigned char *Keystream2;
    //FILE *outFile;
    //outFile = fopen("en_password.txt","w");
    /*
    char input[33] = "";
    scanf("%s", input);
    unsigned char* one = (unsigned char*)malloc(sizeof(unsigned char) * 17);
    unsigned char* two = (unsigned char*)malloc(sizeof(unsigned char) * 17);
    for(int i = 0; i < 16; i++) {
      int tmp1 = input[2 * i];
      int tmp2 = input[2 * i + 1];
      if(tmp1 <= '9' && tmp2 <= '9')
          one[i] = (tmp1 - '0') * 16 + (tmp2 - '0');
      else if(tmp1 <= '9' && tmp2 >= 'a')
          one[i] = (tmp1 - '0') * 16 + (tmp2 - 'a' + 10);
      else if(tmp1 >= 'a' && tmp2 <= '9')
          one[i] = (tmp1 - 'a' + 10) * 16 + (tmp2 - '0');
      else
          one[i] = (tmp1 - 'a' + 10) * 16 + (tmp2 - 'a' + 10);
      two[15 - i] = one[i];
    }*/
    //printf("%s\n%s\n", one, two);
    //Initialization(one, two);
    
    Initialization("abcdefghijklmnop","ponmlkjihgfedcba");
    GenerateKeystream(Keystream,ITEMLENGTH/4);
    getPw();
    Keystream2=(unsigned char*)Keystream;
    encrypt(Keystream2);
    for(i = 0; i < ITEMLENGTH; i++) {
        int tmp = cipher[i];
        if(tmp / 16 < 10)
            cipher0[i * 2] = tmp / 16 + '0';
        else
            cipher0[i * 2] = tmp / 16 - 10 + 'a';
        if(tmp % 16 < 10)
            cipher0[i * 2 + 1] = tmp % 16 + '0';
        else
            cipher0[i * 2 + 1] = tmp % 16 - 10 + 'a';
    }
    /*for(i=0;i<ITEMLENGTH;i++)
        printf("\\x%02x",cipher[i]);
    printf("\n");*/
    for(i = 0; i < ITEMLENGTH * 2 - 4; i++)
        printf("%c", cipher0[i]);
    printf("\n");
        //fprintf(outFile,"\\x%02x",cipher[i]);
    //fclose(outFile);
    return 0;   
}


















