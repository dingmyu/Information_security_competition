...
void UART1_Open_Set() {	//声明设置串口的结构体
    ...
    option.c_iflag &= ~INPCK;// 设置停止位 
    option.c_cflag &= ~CSTOPB;  //8位数据位 无校验位 1位停止位
...
static int _sql_callsearch(void *notused, int argc, char **argv, char **szColName) {  //辅助数据库函数
    printf("%s\n", argv[2] == 0 ? "NULL" : argv[2]);
    return 0;
}

static int _sql_callfuc2(void *tmp, int argc, char **argv, char **szColName) {  //辅助数据库函数
...
int main() {
    ...
    int fd = open("/dev/ttyUSB1", O_RDWR|O_NOCTTY|O_NDELAY);//打开端口
