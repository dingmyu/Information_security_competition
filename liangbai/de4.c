...
unsigned char de_password[ITEMLENGTH],cipher[ITEMLENGTH];
...
/*void getCipher()
{
    ...
    int i=0;
    unsigned char a,b;
    fp=fopen("en_password.txt","r");
    for(i=0;i<ITEMLENGTH;i++)
    {
       a=fgetc(fp);
       b=fgetc(fp);
       fscanf(fp,"%x",&cipher[i]);
    }
    fclose(fp);
}*/
void getCipher() {
    ...
    for(int i = 0; i < ITEMLENGTH; i++){
        char tmp1 = cipher0[2 * i];
        char tmp2 = cipher0[2 * i + 1];
        if(tmp1 <= '9' && tmp2 <= '9')
            cipher[i] = (tmp1 - '0') * 16 + (tmp2 - '0');
        else if(tmp1 <= '9' && tmp2 >= 'a')
            cipher[i] = (tmp1 - '0') * 16 + (tmp2 - 'a' + 10);
        else if(tmp1 >= 'a' && tmp2 <= '9')
            cipher[i] = (tmp1 - 'a' + 10) * 16 + (tmp2 - '0');
        else
            cipher[i] = (tmp1 - 'a' + 10) * 16 + (tmp2 - 'a' + 10);
    }
}
...
int main()
{
    ...
    //FILE *outFile;
    //outFile = fopen("de_password.txt","w");
    ///*
    char input[33] = "";
    scanf("%s", input);
    ...
    for(int i = 0; i < 16; i++) {
      ...
      if(tmp1 <= '9')
          ...
          one[i] = 6 * 16 + (tmp1 - 'a' + 11);
      two[15 - i] = one[i];
    }
    Initialization((u8*)one, (u8*)two);
    //*/
    //Initialization("cahlglbinknhlnop","ponlhnkniblglhac");
    GenerateKeystream(Keystream,ITEMLENGTH/4);
    getCipher();
    Keystream2=(unsigned char*)Keystream;
    decrypt(Keystream2);
    //fprintf(outFile,"%s",de_password);
    for(int i = 0; i < ITEMLENGTH; i++)
      printf("%c", de_password[i]);
    //fclose(outFile);
    return 0;   
}
