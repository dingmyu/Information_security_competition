...
int SM2_Test_Vecotor2()
{
	...
	BIGNUM *x, *y, *z;
	EC_KEY	*eckey = NULL;
	unsigned char	*signature,aa,bb;
	unsigned char	digest[32]; //message digest
	int	sig_len,j;
        FILE *digfp=NULL,*sigfp=NULL;
...
// 	size_t inlen = 6;
	...
	size_t i;
        digfp=fopen("digest.txt","r");
        sigfp=fopen("signature.txt","w"); 
        for(j=0;j<32;j++)
        {
             fscanf(digfp,"%c",&aa);
             fscanf(digfp,"%c",&bb);
             fscanf(digfp,"%x",&digest[j]);
        }
	CRYPTO_set_mem_debug_functions(0, 0, 0, 0, 0);
	CRYPTO_mem_ctrl(CRYPTO_MEM_CHECK_ON);
	ERR_load_crypto_strings();
	...
	if (!BN_hex2bn(&x, "421DEBD61B62EAB6746434EBC3CC315E32220B3BADD50BDC4C4E6C147FEDD43D")) ABORT;//xG
	if (!EC_POINT_set_compressed_coordinates_GFp(group, P, x, 0, ctx)) ABORT;
	if (!EC_POINT_is_on_curve(group, P, ctx)) ABORT;
	if (!BN_hex2bn(&z, "8542D69E4C044F18E8B92435BF6FF7DD297720630485628D5AE74EE7C32E79B7")) ABORT;//n
	if (!EC_GROUP_set_generator(group, P, z, BN_value_one())) ABORT;
	...
	fprintf(stdout, "\n");
	/* G_y value taken from the standard: */
	if (!BN_hex2bn(&z, "0680512BCBB42C07D47349D2153B70C4E5D7FDFCBFA36EA1A85841B9E46E09A2")) ABORT;//yG
	if (0 != BN_cmp(y, z)) ABORT;
	...
	/* create key */
	if (!BN_hex2bn(&z, "128B2FA8BD433C6C068C8D803DFF79792A519A55171B1B650C23661D15897263")) ABORT;//A____z:random number pri_key P:pub_key
	...
	if (!BN_hex2bn(&z, "6CB28D99385C175C94F94E934817663FC176D925DD72B727260DBAAE1FB2F96F")) ABORT;//z:random number Q:point
...
// 	printf("\n");
	...
	fprintf(stdout, "ECSign OK\n");//signature can be written to txt(char)
        for(i=0;i<sig_len;i++)
        {
               printf("%02x",signature[i]);
               fprintf(sigfp,"%02x",signature[i]);
        }
	/* verify signature */
        /* This can be seen in getB
